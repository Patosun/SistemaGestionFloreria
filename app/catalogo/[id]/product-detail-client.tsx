    ...
    <div className="min-h-screen bg-[#FDF3F6] font-sans text-[#93276F] selection:bg-[#E6A1B8] selection:text-[#93276F]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 mb-24">
          ...
          {/* Lado Izquierdo: Imagen Grande + miniaturas */}
            ...
            <div className="relative aspect-square md:aspect-[4/5] rounded-2xl overflow-hidden bg-[#DFD2E5] shadow-lg border border-[#E6A1B8]/20">
              {activeImage ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-[#DFD2E5]">
                  <span className="text-[#93276F]/20 text-8xl font-serif">✿</span>
                </div>
              )}
              <div className="absolute top-4 right-4 flex flex-col gap-2">
                <button className="bg-white/80 backdrop-blur-sm p-3 rounded-full shadow-md text-[#93276F] hover:text-[#E6A1B8] transition-colors">
                  <Heart size={20} />
                </button>
                <button
                  onClick={handleShare}
                ...
                {allImages.map((img, i) => (
                  <button
                    ...
                    className={`shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${
                      activeImage === img ? "border-[#93276F]" : "border-transparent opacity-60 hover:opacity-100"
                    }`}
                  >
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="text-4xl font-light text-[#93276F] mb-8 border-b border-[#E6A1B8]/30 pb-8">
              {product.price.toFixed(0)} <span className="text-xl">BOB</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <div className="flex items-center justify-between border border-[#E6A1B8] rounded-full px-4 py-3 bg-white w-full sm:w-32 shadow-inner">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="text-[#93276F] hover:text-[#E6A1B8]">
                  <Minus size={18} />
                </button>
                <span className="font-bold text-[#93276F]">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} className="text-[#93276F] hover:text-[#E6A1B8]">
                  <Plus size={18} />
                </button>
              </div>
            ...
            <a
              href={`https://wa.me/59177793200?text=Hola,%20me%20interesa%20el%20${encodeURIComponent(product.name)}%20(Cantidad:%20${quantity})`}
              target="_blank"
          ...
          <div className="pt-16 border-t border-[#E6A1B8]/20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {relatedProducts.map((relProduct) => (
                <Link
                  href={`/catalogo/${relProduct.id}`}
                  key={relProduct.id}
                  className="group bg-white rounded-xl overflow-hidden shadow-sm border border-[#E6A1B8]/10 hover:shadow-xl transition-all duration-300"
                >
                  <div className="aspect-square bg-[#DFD2E5] overflow-hidden">
                    {relProduct.image ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <span className="text-[#93276F]/20 text-4xl font-serif">✿</span>
                      </div>
                    )}
                  </div>
                  <div className="p-6 text-center">
                    <h3 className="font-serif text-[#93276F] text-lg mb-2 truncate group-hover:text-[#E6A1B8] transition-colors">
                      {relProduct.name}
                    </h3>
                    <p className="font-bold text-[#E6A1B8]">{relProduct.price.toFixed(0)} BOB</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        {isShareModalOpen && (
          ...
          >
            ...
            >
              ...
              <div>
                <h3 className="text-xs uppercase tracking-widest font-bold text-[#E6A1B8] mb-3">Compartir en</h3>
                <div className="grid grid-cols-2 gap-3">
                  <a
                    href={shareLinks.whatsapp}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 bg-[#25D366] text-white rounded-full py-3 text-xs font-bold uppercase tracking-widest hover:opacity-90 transition-opacity"
                  >
                    WhatsApp
                  </a>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 bg-[#1877F2] text-white rounded-full py-3 text-xs font-bold uppercase tracking-widest hover:opacity-90 transition-opacity"
                  >
                    Facebook
                  </a>
                </div>
